---
layout: course
title: Math101
description: Calculus 2.
year: 2024
course_id: math-101
---

Note: This is an unofficial resource page. These materials are intended as a supplement to our official course content to help clarify complex topics.

## Interactive Tools

I often develop small tools to help visualize the abstract concepts we discuss in small class. Please note that most of the underlying code for these tools is AI-assisted.

---

- Week 4 | Solids of Revolution: An interactive visualization tool to help you "see" the volume generated when a curve is rotated around an axis. [Link](https://math101tools-zrpcpkusl3remw3adoesvf.streamlit.app/)

## Notes

While preparing for our classes, I often create handwritten notes that summarize the day’s key points or explore "extra" content that we might not have time to cover in depth during the lecture. I will upload these periodically for those who want a different perspective on the material.

---

## External resourse

Beyond our course materials, these are some of the best online resources I recommend for mastering certain topics. (Or some tips and tricks)

**All credit go to the owner.**

---

- Week 5 | Integration by parts. The LIATE rule [Video](https://www.youtube.com/watch?v=FS3ksSmdE90)
